...
class RegisterForm extends React.Component {
  ...
  server = () =>{
    ...
    const footerList = ls.get("footerList");
    
   /*  for(var i=0;i<footerList.length;i++){
      if(footerList[i].id === 515256780384121){
          this.props.history.push("/site?index="+i) 
      }
    } */
    const url = window.location.pathname.replace('register', 'site?index=2')
