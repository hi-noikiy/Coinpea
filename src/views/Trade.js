...
//utils
import { addTotal, times, shallowEqualImmutable, _LocalStorage, setSymbol, setPrecision, createdSymbol, numToString } from '../utils/index';
...
//自定义组件
import TradeTabsList from '../components/trade/TradeTabsList.js';
import TradePriceList from '../components/trade/TradePriceList.js';
import NewTradeList from '../components/trade/NewTradeList.js';
import TradeInpPannel from '../components/trade/TradeInpPannel.js';
import DelegateTabsPannel from '../components/trade/DelegateTabsPannel.js';
import TradeviewPage from '../tradingview/TradeviewPage.js';
import ContralTitle from '../components/shared/ContralTitle.js';
import DeepChart from '../components/trade/DeepChart.js';


/* action */
import { SAVE_COIN_LOCAL_ACTION, GET_COIN_DATA, EMIT_UPDATA_COINAREA, DIS_CHANGE_COIN , SAVE_EXCHANGE_RATA_EX} from '../components/trade/TradeTabsListRedux.js';
import { ACTION_ALL_DELE_DATA, ACTION_HIS_DELE_DATA, UPDATE_DELEGATE_CUR,  UPDATE_DELEGATE_HIS} from '../components/trade/DelegateTavsPannelRedux.js';
...
class Trade extends Component {
    ...
    handleData = (data) => {
            ...
            if(result.hasOwnProperty('type')) {
                ...
                this.getPrice('salePrice', saleP[saleP.length-1] ? saleP[saleP.length-1].pr: 0);
                this.getPrice('buyPrice', buyP[0]? buyP[0].pr : 0);
               ...
               /*  if(result.real) {
                ...
                if(result.record) {
                    this.setState({
                        data:this.state.data.update('record', v => v.unshift(result.record))
                    })
                } */
    ...
    delegateCur = (result) => {
      ...
      /*   result.entrust.forEach( (item, index) => {
            item.key = index;
            item.control ="撤销"
        });
          //底部委托数据
