...
import { Map  , List, forEach as _forEach , filter} from 'immutable';
import { sortBy } from 'lodash';
import { times, setPrecision ,createdSymbol, numToString} from '../../utils/index';
...
export function* fetchTabs(action) {
    ...
    const data = yield call(userList.bind(this)); //指示中间件调用fetch异步
                 ...
                 v.data.forEach( o => {
                    ...
                    //添加汇率字段
                     if(state.exRate.get(o.ar)) {
                        ...
                        let val = times( ls.get('lang') === 'en_US'? 1: exRate , times(state.exRate.get(o.ar), o.np));
...
//处理自选 
function formatSelect(areaIndex,id, select, data) {
...
function updateCoin(state,coin, data) {
    ...
    console.log(data)
    //计算新的汇率
    let val = times(times(state.exRate.get(data.ar), data.np),  ls.get('lang') === 'en_US'? 1: exRate);
...
function tradeList(state = initState, action) {
    ...
    switch(action.type) {
        ...
        case SAVE_COIN_LOCAL: {
            return {
                ...state,
                tradeList:formatSelect(action.params.areaIndex,action.params.id, action.params.select, state.tradeList)
            }
        }
