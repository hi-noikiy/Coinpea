...
//api
import { createApi, delectAll, delectOne, mySerect, editSerect, googleStatus } from '../../api/personal';
...
export  class API extends Component {
    ...
    showModal() {          
        ...
        if(this.state.withdrawOpenGA){
            this.setState({
                visible:true
            })
        }else if(this.state.gooleStauts && !this.state.withdrawOpenGA){
            this.setState({
                visible3:true,
                tipText:intl.get('谷歌认证未开启'),
                btnText:intl.get("去开启"),
                path:'/personal'
            })
        }else if(!this.state.gooleStauts){
            this.setState({
                visible3:true,
                tipText:intl.get('未进行谷歌认证'),
                btnText:intl.get("去认证"),
                path:'/googlecheck'
            })
        }
    }
                    ...
                    <div className="key-con">
                        <div className="key-con-info clear">
                            ...
                            <div className="key-con-info-data">
                                ...
                                <p><a href="javascript:;" disabled={item.editing} onClick={this.secretShow.bind(this,index,item.secretShowing)}>{item.secretShowing?intl.get('隐藏'):intl.get('展示')}</a><span className={item.secretShowing?'':'hide'}>{item.secretKey}</span></p>
                                <h6>Option: </h6>
                        ...
                        <div className="limit clear">
                            ...
                            <div className="radio">
                                ...
                                <div className="radio-con">
                                    ...
                                    <p>{intl.get('可信IP：')}</p>
                                    <p><Input disabled={item.editing} placeholder="0.0.0.0" value={item.ipValue} onChange={e=>this.limitIp(e,index)} /></p>
            ...
            <div className="api">
                <div className="api-top clear">
                    <div className="api-top-left">
                        <h4>{intl.get('备注')}</h4>
                        <Input placeholder={intl.get('请输入API key备注名')} value={this.state.key} onChange={e=>this.setState({ key:e.target.value })} />
                        ...
                        >
                        <h5>{intl.get('验证谷歌验证码')}</h5>
                        <Input  placeholder={intl.get("请输入谷歌验证码")} maxLength="6"  value={this.state.googleKey} onChange={e=>this.changeGoogle(e)} />
                        </Modal>
                        {/*<Modal
                        ...
                        >
                        <h5>验证手机验证码</h5>
                        <Input ref="googleKey" placeholder="请输入手机验证码" />
                        </Modal>
                        <Modal
                    ...
                    <div className="api-top-right">
                        <h4>{intl.get('提示')}</h4>
                        <p>• 	{intl.get('api提示1')}<span>{intl.get('API DOC')}</span>{intl.get('api提示1-1')}</p>
                        <p>• 	{intl.get('api提示2')}</p>
                        <p>• 	<span>{intl.get('不要泄露')}</span>{intl.get('api提示3')}</p>
                    </div>
                </div>
    ...
    limitIp(event,num){
        const arr = this.state.secretKeyList.map((item,index)=>{
            if(index === num){
                item.ipValue = event.target.value;
            }
            return item
        });
        this.setState({
